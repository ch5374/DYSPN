...
class RiemannianGradient(torch.autograd.Function):
	...
	def backward(ctx, grad_output):
		(x,) = ctx.saved_tensors
		scale = (1 - RiemannianGradient.c * x.pow(2).sum(-1, keepdim=True)).pow(2) / 4 # 불안정성 원인..
		# print("RiemannianGradient Scale", scale)
...
def _project(x, c):
	norm = torch.clamp_min(x.norm(dim=-1, keepdim=True, p=2), 1e-5)  # 1e-5 보다 작으면 1e-5로 ! 아니면 norm 그대로 냅둠
	maxnorm = (1 - 1e-3) / (c ** 0.5)
	cond = norm > maxnorm
	projected = x / norm * maxnorm
	return torch.where(cond, projected, x)  # cond 조건이 맞으면 projected/ False면 x
...
def _expmap0(u, c):
	sqrt_c = c ** 0.5
	u_norm = torch.clamp_min(u.norm(dim=-1, p=2, keepdim=True), 1e-5)
	gamma_1 = tanh(sqrt_c * u_norm) * u / (sqrt_c * u_norm + 1e-8)
	return gamma_1
...
def _logmap0(y, c):
	sqrt_c = c ** 0.5
	y_norm = torch.clamp_min(y.norm(dim=-1, p=2, keepdim=True), 1e-5) + 1e-8
...
def _mobius_matvec(m, x, c):
	...
	mx = x @ m.to(x.device).transpose(-1, -2)
	...
	# print(mx_norm.size())
	# input()
	# print(mx_norm * sqrt_c)
	# torch.set_printoptions(profile='short')
	res_c = tanh(mx_norm / x_norm * artanh(sqrt_c * x_norm)) * mx / ((mx_norm * sqrt_c) + 1e-8)  # 여기가 0으로 나누고 있었음.....
...
"""
0416 nonlinearity 추가
"""
def mobius_fn_apply(fn, x,c=0.1):
	r"""
