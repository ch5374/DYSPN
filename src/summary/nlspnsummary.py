...
class NLSPNSummary(BaseSummary):
    ...
    def update(self, global_step, sample, output):
        ...
        if output['confidence'] is not None:
            confidence = output['confidence'].data.cpu().numpy().any()
        else:
            confidence = None
        ...
        with torch.no_grad():
            ...
            if self.args.save_result_only:
                ...
                if self.args.model_name == 'DYSPN':
                    visual_attention(output['attention'], self.path_output)

            else:
                # Parse data
                if output['guidance'] != None:
                    guidance = output['guidance'].data.cpu().numpy().any()
                else:
                    guidance = None

                if output['pred_init'] != None:
...
def visual_attention(x, basepath):
    x = x.data.cpu()
    x = x[0].numpy()
    fig, ax = plt.subplots(4, 6, figsize=(80, 60))
    ...
    plt.show()
    plt.savefig('{}/attention.png'.format(basepath))